...
# Mock RSpec Matchers for change ... and
# Wait, `change` and `not_change` are standard RSpec matchers?
# RSpec has `change`, but `not_change` is not standard RSpec (it's RSpec 3 standard but often written as `not_to change`).
# Mastodon defines a custom `not_change` matcher or uses a gem.
# Let's define `not_change` to alias `change` and invert it? Or just define it as a custom matcher.
RSpec::Matchers.define :not_change do |receiver, message|
  match do |event|
    if block_arg
      old_val = block_arg.call
      event.call
      new_val = block_arg.call
    else
      old_val = receiver.send(message)
      event.call
      new_val = receiver.send(message)
    end
    old_val == new_val
  end
