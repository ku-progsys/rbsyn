...
module Hamster
  ...
  class Vector
    ...
    # `Hash_1`es, non-existing keys are created with empty `Hash_1` values.
    #
    ...
    #
    # Note that the original `Vector` and sub-`Vector`s and sub-`Hash_1`es are
    # left unmodified; new data structure copies are created along the path
    # wherever needed.
    #
    # @example
    #   v = Hamster::Vector[123, 456, 789, Hamster::Hash_1["a" => Hamster::Vector[5, 6, 7]]]
    #   v.update_in(3, "a", 1) { |value| value + 9 }
    #   # => Hamster::Vector[123, 456, 789, Hamster::Hash_1["a" => Hamster::Vector[5, 15, 7]]]
    #
