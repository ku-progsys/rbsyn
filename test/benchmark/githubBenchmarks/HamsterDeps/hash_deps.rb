...
module Hamster
  ...
  # A `Hamster::Hash_1` maps a set of unique keys to corresponding values, much
  ...
  # Ruby's built-in Hash_1, which we will call RubyHash_1 for clarity. Like
  # RubyHash_1, two keys that are `#eql?` to each other and have the same
  # `#hash` are considered identical in a `Hamster::Hash_1`.
  #
  # A `Hamster::Hash_1` can be created in a couple of ways:
  #
  #     Hamster::Hash_1.new(font_size: 10, font_family: 'Arial')
  #     Hamster::Hash_1[first_name: 'John', last_name: 'Smith']
  #
  # Any `Enumerable` object which yields two-element `[key, value]` arrays
  # can be used to initialize a `Hamster::Hash_1`:
  #
  #     Hamster::Hash_1.new([[:first_name, 'John'], [:last_name, 'Smith']])
  #
  # Key/value pairs can be added using {#put}. A new hash is returned and the
  # existing one is left unchanged:
  #
  #     hash = Hamster::Hash_1[a: 100, b: 200]
  #     hash.put(:c, 500) # => Hamster::Hash_1[:a => 100, :b => 200, :c => 500]
  #     hash              # => Hamster::Hash_1[:a => 100, :b => 200]
  #
  # {#put} can also take a block, which is used to calculate the value to be
  # stored.
  #
  #     hash.put(:a) { |current| current + 200 } # => Hamster::Hash_1[:a => 300, :b => 200]
  #
  # Since it is immutable, all methods which you might expect to "modify" a
  # `Hamster::Hash_1` actually return a new hash and leave the existing one
  # unchanged. This means that the `hash[key] = value` syntax from RubyHash_1
  # *cannot* be used with `Hamster::Hash_1`.
  #
  # Nested data structures can easily be updated using {#update_in}:
  #
  #     hash = Hamster::Hash_1["a" => Hamster::Vector[Hamster::Hash_1["c" => 42]]]
  #     hash.update_in("a", 0, "c") { |value| value + 5 }
  #     # => Hamster::Hash_1["a" => Hamster::Hash_1["b" => Hamster::Hash_1["c" => 47]]]
  #
  # While a `Hamster::Hash_1` can iterate over its keys or values, it does not
  # guarantee any specific iteration order (unlike RubyHash_1). Methods like
  # {#flatten} do not guarantee the order of returned key/value pairs.
  #
  # Like RubyHash_1, a `Hamster::Hash_1` can have a default block which is used
  # when looking up a key that does not exist. Unlike RubyHash_1, the default
  # block will only be passed the missing key, without the hash itself:
  #
  #     hash = Hamster::Hash_1.new { |missing_key| missing_key * 10 }
  #     hash[5] # => 50
  class Hash_1
    include Immutable
    include Enumerable
    ...
    class << self
      ...
      #
      # @example
      #   Hamster::Hash_1["A" => 1, "B" => 2] # => Hamster::Hash_1["A" => 1, "B" => 2]
      #   Hamster::Hash_1[["A", 1], ["B", 2]] # => Hamster::Hash_1["A" => 1, "B" => 2]
      #
      ...
      # Return an empty `Hash_1`. If used on a subclass, returns an empty instance
      # of that class.
      #
      # @return [Hash_1]
      def empty
      ...
      # "Raw" allocation of a new `Hash_1`. Used internally to create a new
    ...
    # Return the number of key/value pairs in this `Hash_1`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].size  # => 3
    #
    # @return [Integer]
    def size
      @trie.size
    ...
    # Return `true` if this `Hash_1` contains no key/value pairs.
    #
    # @return [Boolean]
    def empty?
      @trie.empty?
    ...
    # Return `true` if the given key object is present in this `Hash_1`. More precisely,
    # return `true` if a key with the same `#hash` code, and which is also `#eql?`
    # to the given key object is present.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].key?("B")  # => true
    #
    ...
    # Return `true` if this `Hash_1` has one or more keys which map to the provided value.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].value?(2)  # => true
    #
    ...
    # this `Hash_1` has a default block, the default block is called to provide the
    # value. Otherwise, return `nil`.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h["B"]             # => 2
    #   h.get("B")         # => 2
    #   h.get("Elephant")  # => nil
    #
    #   # Hamster Hash_1 with a default proc:
    #   h = Hamster::Hash_1.new("A" => 1, "B" => 2, "C" => 3) { |key| key.size }
    #   h.get("B")         # => 2
    #   h.get("Elephant")  # => 8
    #
    ...
    # Return a new `Hash_1` with the existing key/value associations, plus an association
    ...
    # frozen. This matches RubyHash_1's behaviour.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2]
    #   h.put("C", 3)
    #   # => Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h.put("B") { |value| value * 10 }
    #   # => Hamster::Hash_1["A" => 1, "B" => 20]
    #
    ...
    # @raise NoMethodError
    def []=(*)
      raise NoMethodError, "Hamster::Hash_1 doesn't support `[]='; use `put' instead"
    end

    # Return a new `Hash_1` with a deeply nested value modified to the result of
    # the given code block.  When traversing the nested `Hash_1`es and `Vector`s,
    # non-existing keys are created with empty `Hash_1` values.
    #
    ...
    #
    # Note that the original `Hash_1` and sub-`Hash_1`es and sub-`Vector`s are left
    # unmodified; new data structure copies are created along the path wherever
    # needed.
    #
    # @example
    #   hash = Hamster::Hash_1["a" => Hamster::Hash_1["b" => Hamster::Hash_1["c" => 42]]]
    ...
    # An alias for {#put} to match RubyHash_1's API. Does not support {#put}'s
    # block form.
    #
    # @see #put
    # @param key [Object] The key to store
    ...
    # Return a new `Hash_1` with `key` removed. If `key` is not present, return
    # `self`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].delete("B")
    #   # => Hamster::Hash_1["A" => 1, "C" => 3]
    #
    ...
    # Call the block once for each key/value pair in this `Hash_1`, passing the key/value
    # pair as parameters. No specific iteration order is guaranteed, though the order will
    # be stable for any particular `Hash_1`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].each { |k, v| puts "k=#{k} v=#{v}" }
    #
    #   k=A v=1
    #   k=C v=3
    #   k=B v=2
    ...
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].reverse_each { |k, v| puts "k=#{k} v=#{v}" }
    #
    #   k=B v=2
    #   k=C v=3
    #   k=A v=1
    ...
    # Call the block once for each key/value pair in this `Hash_1`, passing the key as a
    # parameter. Ordering guarantees are the same as {#each}.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].each_key { |k| puts "k=#{k}" }
    #
    #   k=A
    #   k=C
    #   k=B
    ...
    # Call the block once for each key/value pair in this `Hash_1`, passing the value as a
    # parameter. Ordering guarantees are the same as {#each}.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].each_value { |v| puts "v=#{v}" }
    #
    #   v=1
    #   v=3
    #   v=2
    ...
    # All the returned `[key, value]` arrays will be gathered into a new `Hash_1`.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h.map { |k, v| ["new-#{k}", v * v] }
    #   # => Hash_1["new-C" => 9, "new-B" => 4, "new-A" => 1]
    #
    # @yield [key, value] Once for each key/value pair.
    # @return [Hash_1]
    def map
    ...
    # Return a new `Hash_1` with all the key/value pairs for which the block returns true.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h.select { |k, v| v >= 2 }
    #   # => Hamster::Hash_1["B" => 2, "C" => 3]
    #
    # @yield [key, value] Once for each key/value pair.
    # @yieldreturn Truthy if this pair should be present in the new `Hash_1`.
    # @return [Hash_1]
    def select(&block)
    ...
    #
    # `other` can be a `Hamster::Hash_1`, a built-in Ruby `Hash_1`, or any `Enumerable`
    # object which yields `[key, value]` pairs.
    #
    # @example
    #   h1 = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h2 = Hamster::Hash_1["C" => 70, "D" => 80]
    #   h1.merge(h2)
    #   # => Hamster::Hash_1["C" => 70, "A" => 1, "D" => 80, "B" => 2]
    #   h1.merge(h2) { |key, v1, v2| v1 + v2 }
    #   # => Hamster::Hash_1["C" => 73, "A" => 1, "D" => 80, "B" => 2]
    #
    # @param other [::Enumerable] The collection to merge with
    # @yieldparam key [Object] The key which was present in both collections
    # @yieldparam my_value [Object] The associated value from this `Hash_1`
    # @yieldparam other_value [Object] The associated value from the other collection
    # @yieldreturn [Object] The value to associate this key with in the new `Hash_1`
    # @return [Hash_1]
    def merge(other)
      trie = if block_given?
    ...
    # this `Hash_1` as two-element `Array`s.
    #
    # @overload sort
    ...
    #
    #   @example
    #     h = Hamster::Hash_1["Dog" => 1, "Elephant" => 2, "Lion" => 3]
    ...
    # Return a {Vector} which contains all the `[key, value]` pairs in this `Hash_1`
    ...
    # Return a new `Hash_1` with the associations for all of the given `keys` removed.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h.except("A", "C")  # => Hamster::Hash_1["B" => 2]
    #
    ...
    # Return a new `Hash_1` with only the associations for the `wanted` keys retained.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    #   h.slice("B", "C")  # => Hamster::Hash_1["B" => 2, "C" => 3]
    #
    ...
    # If any of the `wanted` keys are not present in this `Hash_1`, `nil` will be
    ...
    # Return a {Vector} of the values which correspond to the `wanted` keys.
    # If any of the `wanted` keys are not present in this `Hash_1`, raise `KeyError`
    # exception.
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3]
    ...
    # Return a new {Set} containing the keys from this `Hash_1`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3, "D" => 2].keys
    #   # => Hamster::Set["D", "C", "B", "A"]
    #
    # @return [Set]
    def keys
    ...
    # Return a new {Vector} populated with the values from this `Hash_1`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3, "D" => 2].values
    ...
    # Return a new `Hash_1` created by using keys as values and values as keys.
    ...
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3, "D" => 2].invert
    #   # => Hamster::Hash_1[1 => "A", 3 => "C", 2 => "B"]
    #
    # @return [Hash_1]
    def invert
      pairs = []
    ...
    # Return a new {Vector} which is a one-dimensional flattening of this `Hash_1`.
    ...
    #
    # @example
    #   h = Hamster::Hash_1["A" => 1, "B" => [2, 3, 4]]
    #   h.flatten
    #   # => Hamster::Vector["A", 1, "B", [2, 3, 4]]
    #   h.flatten(2)
    #   # => Hamster::Vector["A", 1, "B", 2, 3, 4]
    #
    # @param level [Integer] The number of times to recursively flatten the `[key, value]` pairs in this `Hash_1`.
    # @return [Vector]
    def flatten(level = 1)
    ...
    # Searches through the `Hash_1`, comparing `obj` with each key (using `#==`).
    # When a matching key is found, return the `[key, value]` pair as an array.
    # Return `nil` if no match is found.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].assoc("B")  # => ["B", 2]
    #
    ...
    # Searches through the `Hash_1`, comparing `obj` with each value (using `#==`).
    # When a matching value is found, return the `[key, value]` pair as an array.
    # Return `nil` if no match is found.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].rassoc(2)  # => ["B", 2]
    #
    ...
    # Searches through the `Hash_1`, comparing `value` with each value (using `#==`).
    # When a matching value is found, return its associated key object.
    # Return `nil` if no match is found.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].key(2)  # => "B"
    #
    ...
    # Return a randomly chosen `[key, value]` pair from this `Hash_1`. If the hash is empty,
    # return `nil`.
    #
    # @example
    #   Hamster::Hash_1["A" => 1, "B" => 2, "C" => 3].sample
    #   # => ["C", 3]
    #
    # @return [Array]
    def sample
    ...
    # Return an empty `Hash_1` instance, of the same class as this one. Useful if you
    # have multiple subclasses of `Hash_1` and want to treat them polymorphically.
    ...
    # Return true if `other` has the same type and contents as this `Hash_1`.
    #
    ...
    # Return true if this `Hash_1` is a proper superset of `other`, which means
    # all `other`'s keys are contained in this `Hash_1` with the identical
    # values, and the two hashes are not identical.
    #
    # @param other [Hamster::Hash_1] The object to compare with
    # @return [Boolean]
    def >(other)
      self != other && self >= other
    end

    # Return true if this `Hash_1` is a superset of `other`, which means all
    # `other`'s keys are contained in this `Hash_1` with the identical values.
    #
    # @param other [Hamster::Hash_1] The object to compare with
    # @return [Boolean]
    def >=(other)
    ...
    # Return true if this `Hash_1` is a proper subset of `other`, which means all
    ...
    # Return true if this `Hash_1` is a subset of `other`, which means all its
    ...
    # Return the contents of this `Hash_1` as a programmer-readable `String`. If all the
    # keys and values are serializable as Ruby literal strings, the returned string can
    # be passed to `eval` to reconstitute an equivalent `Hash_1`. The default
    ...
    # Allows this `Hash_1` to be printed at the `pry` console, or using `pp` (from the
    ...
    # Convert this `Hamster::Hash_1` to an instance of Ruby's built-in `Hash_1`.
    #
    # @return [::Hash_1]
    def to_hash
      output = {}
    ...
    # Return a new `Hash_1` which is derived from this one, using a modified {Trie}.
    # The new `Hash_1` will retain the existing default block, if there is one.
    #
  ...
  # The canonical empty `Hash_1`. Returned by `Hash_1[]` when
  # invoked with no arguments; also returned by `Hash_1.empty`. Prefer using this
  # one rather than creating many empty hashes using `Hash_1.new`.
  #
  # @private
  EmptyHash_1 = Hamster::Hash_1.empty
end